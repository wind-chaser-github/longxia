...
class ImageGenerationTool(Tool):
    ...
    def _missing_api_key_error(self) -> str:
        ...
        if provider == "volcengine":
            return "Error: Volcengine API key is not configured. Set providers.volcengine.apiKey."
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            import sqlite3
            from conductor.db_models import get_default_db_path
            db_path = get_default_db_path()
            with sqlite3.connect(db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT model_name FROM llm_models_config WHERE scenario = 'image_generation'")
                row = cursor.fetchone()
                if row:
                    model_name = row[0]
        except Exception:

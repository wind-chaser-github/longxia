...
class VolcengineImageGenerationClient:
    """Small async client for Volcengine (Doubao Seedream) image generation."""
    ...
    async def generate(
        ...
        *,
        ...
        if not self.api_key:
            raise ImageGenerationError(
                "Volcengine API key is not configured. Set providers.volcengine.apiKey."
            )

        # 针对火山引擎 Seedream 旗舰模型，若请求分辨率小于 3686400 像素，自动升级为 2048x2048
        target_size = image_size or "2048x2048"
        if target_size in ("1024x1024", "512x512"):
            target_size = "2048x2048"

        # 火山引擎文生图标准请求体 (支持方舟平台 Seedream 系列)
        body: dict[str, Any] = {
            "model": model,
            "prompt": prompt,
            "size": target_size,
        ...
        for item in data.get("data") or []:
            ...
            if isinstance(b64, str) and b64:
                ...
                if isinstance(url_val, str) and url_val:
                    if self._client is not None:
                        images.append(await _download_image_data_url(self._client, url_val))
                    else:
                        async with httpx.AsyncClient(timeout=self.timeout) as dl_client:
                            images.append(await _download_image_data_url(dl_client, url_val))
